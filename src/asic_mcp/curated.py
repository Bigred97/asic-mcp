"""Hand-curated metadata for the top-N ASIC register datasets.
...
- plain-English aliases for ASIC's verbose column names
- which filter values are accepted, what they mean
- search keywords folded into the fuzzy search haystack

The translator turns a user's plain-English `filters={...}` request into
instructions the shaping layer can apply to the parsed DataFrame.
"""
...
class CuratedDimensionValues:
    ...
    Used for state/territory codes, current_status enums, etc. `None` means
    free-form (e.g. licensee_name, adviser_number) — anything goes.
    """
...
class CuratedDataset:
    ...
    max_rows: int | None                     # cap on data rows read — used to carve out a sub-table from a multi-section sheet
    layout: Layout                           # "wide" = entities-as-rows; "transposed" = years-as-cols
    period_coverage: str | None              # e.g. "current snapshot (weekly)"
    update_frequency: str | None             # "weekly", "monthly", "daily"
...
def _yaml_dir() -> Path:
    try:
        ref = resources.files("asic_mcp").joinpath("data/curated")
        if ref.is_dir():
            return Path(str(ref))
    ...
    if here.is_dir():
        return here
    raise FileNotFoundError("Could not locate asic_mcp/data/curated/")
