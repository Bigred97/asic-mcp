...
Ported from rba-mcp. Cache kinds tuned for ASIC's cadence:
- "data": ASIC bulk-download files — 24 hours (registers refresh weekly).
- "latest": short freshness window for `latest()` calls.
- "register": weekly-updated ASIC registers — 24 hours.
...
DEFAULT_DB_PATH = Path.home() / ".asic-mcp" / "cache.db"
...
class Cache:
    ...
    async def get_stale(self, key: str) -> tuple[bytes, float] | None:
        ...
        Mid-session corruption is recovered the same way `get` handles it —
        drop the DB, recreate, and return None (no payload to serve).
        """
        await self._ensure_init()
        try:
            async with aiosqlite.connect(self.db_path) as conn:
