...
Only the latest minor release of `asic-mcp` is supported with security fixes. Older versions should be upgraded.
...
Privately report via GitHub's [Security Advisories](https://github.com/Bigred97/asic-mcp/security/advisories/new) flow, or email `hvass97@gmail.com` with subject `[asic-mcp security]`.

Include:
- A clear description of the vulnerability
- A reproducer (minimal MCP call sequence or input that triggers it)
- The version of `asic-mcp` you tested against (`asic_mcp.__version__`)
...
`asic-mcp` runs locally as an MCP stdio subprocess of your MCP client (Claude Desktop, Cursor, etc.). It:

- Reads no local files except its own SQLite cache at `~/.asic-mcp/cache.db`.
